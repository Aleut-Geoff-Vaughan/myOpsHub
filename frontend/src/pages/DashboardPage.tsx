...
export function DashboardPage() {
  ...
  // Calculate date range based on the selected view and reference date
  const dateRange = useMemo(() => {
    ...
    if (selectedView === 'current-week') {
      ...
      startDate = monday;
      endDate = new Date(monday);
      endDate.setDate(endDate.getDate() + 13); // End of second week
    } else { // month
  ...
  // Fetch all dashboard data in a single optimized API call with proper date range
  const { data: dashboardData, isLoading, error} = useDashboard(user?.id, dateRange.startDate, dateRange.endDate);
  ...
  // Memoized stats calculation using server-calculated values
  const stats = useMemo(() => {
