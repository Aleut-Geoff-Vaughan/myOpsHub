...
export function ModuleRail({ expanded = false, onExpandedChange }: ModuleRailProps) {
  ...
  const { hasForecastAccess, hasSalesOpsAccess, hasAdminAccess } = useModuleAccess();
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 py-4 space-y-1">
        ...
        {/* mySalesOps - conditional on role */}
        {hasSalesOpsAccess && (
          <ModuleButton
            moduleId="salesops"
            isActive={currentModule === 'salesops' && !isAdminOpen}
            onClick={() => navigateToModule('salesops')}
            label="mySalesOps"
            icon={Icons.salesops}
